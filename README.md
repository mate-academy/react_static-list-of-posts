...
- Replace `<your_account>` with your Github username in the
  [DEMO LINK](https://AndriiRut.github.io/react_static-list-of-posts/)
...
2. The `PostInfo` accepts a `post` object (with `user` and `comments`
...
3. The `UserInfo` accepts a `user` object and renders at least a `name` and an
  `email`.
   - Please, add `data-cy="user-name"` and `data-cy="user-email"` attributes
   to `name` and `email` elements.

4. The `CommentInfo` accepts a `comment` object and renders a `name`, a `body`, and an `email`.
